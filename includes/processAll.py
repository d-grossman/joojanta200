...
_MODEL_DEF_FILE = "/work/crfasrnn/python-scripts/TVG_CRFRNN_new_deploy.prototxt.dg"
# Contains the trained weights. Download from http://goo.gl/j7PrPZ
_MODEL_FILE = "/work/model/TVG_CRFRNN_COCO_VOC.caffemodel"
...
def crfrnn_segmenter(model_def_file, model_file, gpu_device, inputs):
    ...
    assert os.path.isfile(
        model_def_file), "File {} is missing".format(model_def_file)
    ...
    print(
        "Time taken to run the network: {:.4f} seconds".format(
            end_time -
            start_time))
...
def run_crfrnn(input_file, output_file, gpu_device):
    ...
    # Get predictions
    segmentation = crfrnn_segmenter(
        _MODEL_DEF_FILE, _MODEL_FILE, gpu_device, [im])
...
def resize_image(image):
    ...
    if max_dim > _MAX_DIM:
        ...
        image = image.resize(
            (int(height * ratio), int(width * ratio)), resample=PILImage.BILINEAR)
        image = np.array(image)
...
def main():
    ...
    if len(sys.argv) != 1:
        run_crfrnn('/work/flask/static/top.jpg',
                   '/work/flask/static/bottom.bmp', -1)
        return 0
